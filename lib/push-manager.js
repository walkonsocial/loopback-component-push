...
PushManager.prototype.configureApplication = function (appId, deviceType, cb) {
  ...
  if (cacheApp[appId] && cacheApp[appId][deviceType]) {
    return process.nextTick(function () {
      cb(null, cacheApp[appId][deviceType]);
    });
  }
...
PushManager.prototype.notifyMany = function(appId, deviceType, deviceTokens, notification, cb) {
    ...
    this.configureApplication(
        ...
        function(err, provider) {
            if (err) { return cb(err); }
            //For GCM, send push notifications in batches
            if (deviceType === 'android' && Array.isArray(deviceTokens)) {
              var maxLimit = 1000;
              var numTokens = deviceTokens.length;
              var startIndex = 0;
              while (startIndex < numTokens) {
                var endIndex = (startIndex + maxLimit);
                if (endIndex >= numTokens) {
                  endIndex = numTokens - 1;
                }
                var subArray = deviceTokens.slice(startIndex, endIndex);
                provider.pushNotification(notification, subArray);
